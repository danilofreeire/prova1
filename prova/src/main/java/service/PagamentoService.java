 ...
 *
 * É o equivalente "organizado" ao que a Tela01 do      fazia.
 */
public class PagamentoService {
    ...
    private final PessoaPagamentoDAO pessoaPagamentoDAO = new PessoaPagamentoDAO();

    public PagamentoService() {
        // Usa a mesma URL que o     
    ...
    public PagamentoResultado processar(PagamentoRequisicao requisicao) throws Exception {
        ...
        // 3. Se a modalidade for PIX, gera o QR Code (igual o      faz na Tela01)
        if ("PIX".equalsIgnoreCase(resultado.getModo())) {
            // No código do     : QRcode.gerarQRCode(p1.getId());
            qrCodeGenerator.gerarQRCode(resultado.getId());
            // Isso vai criar/atualizar o arquivo "QRCodePix.png"
        }
 // MONTA OBJETO PessoaPagamento PARA SALVAR NO BANCO
        PessoaPagamento p = new PessoaPagamento();
        p.setNome(requisicao.getNome());
        p.setFone(requisicao.getFone()); // se tiver telefone na requisição
        p.setCpf(requisicao.getCpf());
        p.setId(resultado.getId());              // id/payload da API
        p.setModo(resultado.getModo());
        p.setRetmsg(resultado.getMensagem());
        p.setValor(resultado.getValor());
        p.setDataHora(LocalDateTime.now());

        pessoaPagamentoDAO.salvar(p);
